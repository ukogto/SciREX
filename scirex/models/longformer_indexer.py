...
_NEVER_LOWERCASE = ['<unk>', '<pad>', '<cls>', '<mask>']
...
class WordpieceIndexer(TokenIndexer[int]):
    def __init__(self,
        ...
        self._start_piece_ids = [wordpieceId
                                 for token in (start_tokens or [])
                                 for wordpieceId in wordpiece_tokenizer(token, add_prefix_space=True)['input_ids'][1:-1]]#remove last and stating ids
        self._end_piece_ids = [wordpieceId
                               for token in (end_tokens or [])
                               for wordpieceId in wordpiece_tokenizer(token, add_prefix_space=True)['input_ids'][1:-1]]#remove last and stating ids

        # Convert the separator_token to wordpiece_ids
        self._separator_ids = wordpiece_tokenizer(separator_token, add_prefix_space=True)['input_ids'][1:-1]#remove last and stating ids
    ...
    def tokens_to_indices(self,
        ...
        token_wordpiece_ids = [[wordpiece_ids for wordpiece_ids in self.wordpiece_tokenizer(token)['input_ids'][1:-1]]
                               for token in text]
        ...
        #padd to wordpiece and offsets and token-type-id
        print(len(wordpiece_ids), len(token_type_ids), len(offsets), "before padding wordpiece id token type id and offsets len")
        #calulate multiple of self.max_pieces
        mult = (len(wordpiece_ids)//self.max_pieces+1)
        wordpiece_ids += [self.wordpiece_tokenizer("<pad>")["input_ids"][1:-1][0] for i in range(0,mult*self.max_pieces-len(wordpiece_ids))]
        token_type_ids +=[0 for i in range(0,mult*self.max_pieces-len(token_type_ids))]
        offsets += [0 for i in range(0,mult*self.max_pieces-len(offsets))]
        print(len(wordpiece_ids), len(token_type_ids), len(offsets), "wordpiece id token type id and offsets len")
        ...
        # print(len(wordpiece_ids), len(offsets), "here is the token indexer of the lf")
...
@TokenIndexer.register("longformer-pretrained")
class PretrainedLongformerIndexer(WordpieceIndexer):
    def __init__(self,
        ...
        if pretrained_model.endswith("-cased") and do_lowercase:
            logger.warning("Your longformer model appears to be cased, "
                           "but your indexer is lowercasing tokens.")
        elif pretrained_model.endswith("-uncased") and not do_lowercase:
            logger.warning("Your longformer model appears to be uncased, "
                           "but your indexer is not lowercasing tokens.")
        longformer_tokenizer = LongformerTokenizer.from_pretrained("longformer_weights")
        with open(pretrained_model, 'r') as vocab_fh:
            vocab = json.load(vocab_fh)
        super().__init__(vocab=vocab,
                         wordpiece_tokenizer=longformer_tokenizer,
    ...
    def __eq__(self, other):
        if isinstance(other, PretrainedLongformerIndexer):
